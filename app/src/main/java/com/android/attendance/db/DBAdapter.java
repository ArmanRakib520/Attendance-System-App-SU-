...
public class DBAdapter extends SQLiteOpenHelper {
    ...
    //facult crud
    public void addFaculty(TeacherBean teacherBean) {
        SQLiteDatabase db = this.getWritableDatabase();

        String query = "INSERT INTO faculty_table (faculty_firstname,faculty_mobilenumber,faculty_address,faculty_username,faculty_password) values ('" +
                teacherBean.getFaculty_firstname() + "', '" +
                teacherBean.getFaculty_mobilenumber() + "', '" +
                teacherBean.getFaculty_address() + "', '" +
                teacherBean.getFaculty_username() + "', '" +
                teacherBean.getFaculty_password() + "')";
        Log.d("query", query);
        db.execSQL(query);
        db.close();
    }

    public TeacherBean validateFaculty(String userName, String password) {
        SQLiteDatabase db = this.getWritableDatabase();
        ...
        if (cursor.moveToFirst()) {
            ...
            teacherBean.setFaculty_id(Integer.parseInt(cursor.getString(0)));
            teacherBean.setFaculty_firstname(cursor.getString(1));
            teacherBean.setFaculty_mobilenumber(cursor.getString(2));
            teacherBean.setFaculty_address(cursor.getString(3));
            teacherBean.setFaculty_username(cursor.getString(4));
            teacherBean.setFaculty_password(cursor.getString(5));
            return teacherBean;
        }
        return null;
    }

    public ArrayList<TeacherBean> getAllFaculty() {
